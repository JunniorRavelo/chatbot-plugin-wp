...
class Chatbot_Provider_Ollama implements Chatbot_AI_Provider {
	...
	public function complete( string $system, array $messages, array $settings ) {
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error(
				'provider_timeout',
				__( 'Could not connect to Ollama.', 'chatbot-plugin-wp' ),
		...
		if ( $code < 200 || $code >= 300 ) {
			return new WP_Error(
				'provider_upstream',
				__( 'Ollama returned an error.', 'chatbot-plugin-wp' ),
		...
		if ( '' === $text ) {
			return new WP_Error(
				...
				__( 'Ollama did not return a valid response.', 'chatbot-plugin-wp' ),
